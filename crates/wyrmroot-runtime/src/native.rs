...
use deepwyrm_syscall::{
    ...
    DW_SYSCALL_CLOCK_GET, DW_TASK_TERMINATION_INFO_V1_SIZE, DW_WAIT_MODE_ANY,
    DW_WAIT_RESULT_V1_SIZE, DwAddressRegionMapArgsV1, DwAddressRegionMapFlags,
    DwChannelReceiveResultV1, DwDeadline, DwHandle, DwHandleTransferV1, DwMemoryObjectInfoV1,
    DwObjectInfoV1, DwObjectType, DwOffset, DwReceivedHandleInfoV1, DwRights, DwSize, DwStatus,
    DwTaskTerminationInfoV1, DwUserAddress, DwWaitItemV1, DwWaitResultV1,
};
...
#[allow(
    unsafe_code,
    reason = "the pinned Deepwyrm syscall crate currently exposes the generated veneer but no typed CLOCK_GET wrapper"
)]
unsafe extern "C" {
    /// Generated SysV-to-Deepwyrm syscall veneer supplied by `deepwyrm-syscall`.
    fn dw_syscall6(
        number: u64,
        arg0: u64,
        arg1: u64,
...
pub fn monotonic_active_now() -> Result<u64, NativeError> {
    ...
    let status = raw_clock_get(DW_CLOCK_MONOTONIC_ACTIVE.0, &mut nanoseconds);
    require_success(DwStatus(status as i32))?;
    Ok(nanoseconds)
}

#[allow(
    unsafe_code,
    reason = "the output pointer is a live aligned u64 for the complete generated CLOCK_GET call"
)]
fn raw_clock_get(clock_id: u32, out_nanoseconds: &mut u64) -> i64 {
    // SAFETY: `DW_SYSCALL_CLOCK_GET` and the clock-domain scalar are generated by the pinned ABI;
    // the mutable output reference remains valid and uniquely borrowed throughout the veneer call.
    unsafe {
        dw_syscall6(
            u64::from(DW_SYSCALL_CLOCK_GET.0),
            u64::from(clock_id),
            core::ptr::from_mut(out_nanoseconds) as u64,
            0,
            0,
            0,
            0,
