...
pub fn load_service_process<P: LoaderPlatform>(
    ...
    load_service_process_internal(platform, authority, request)
}

fn load_service_process_internal<P: LoaderPlatform>(
    platform: &mut P,
    authority: LoadAuthority,
    request: ServiceLoadRequest<'_>,
) -> Result<LoadedProcess, ServiceLoadError<P::Error>> {
...
fn load_process_internal<P: LoaderPlatform>(
    ...
    fault: LoadFault,
    preserve_failed_init_channels: bool,
    ...
    if let Err(cause) = platform.send_init(
        ...
        if preserve_failed_init_channels {
